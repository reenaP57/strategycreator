...
extension UIView {
    ...
    func addSubview(_ view:UIView?, marginInsets:UIEdgeInsets, Index: Int) {
        if let view = view {
    ...
    func insertSubview(_ view:UIView?, marginInsets:UIEdgeInsets, Index: Int) {
        if let view = view {
            view.translatesAutoresizingMaskIntoConstraints = false
            ...
            self.addConstraint(NSLayoutConstraint(item: view, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .height, multiplier: 1.0, constant: view.CViewHeight))
